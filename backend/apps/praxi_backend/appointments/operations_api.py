...
from .scheduling_facade import plan_operation as scheduling_plan_operation
...
class OpDashboardView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response(
            {"date": date_obj.isoformat(), "operations": data}, status=status.HTTP_200_OK
        )
...
class OpDashboardStatusUpdateView(generics.GenericAPIView):
    ...
    def patch(self, request, pk: int, *args, **kwargs):
        ...
        if not allowed:
            _audit(
                ok=False, detail="invalid_transition", meta={"from": old_status, "to": new_status}
            )
            return Response(
        ...
        if new_status == Operation.STATUS_RUNNING and now < obj.start_time:
            _audit(
                ok=False, detail="running_before_start", meta={"from": old_status, "to": new_status}
            )
            return Response(
...
class OperationSuggestView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if primary_surgeon is None:
            return Response(
                {"detail": "primary_surgeon_id not found."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not getattr(op_type, "active", True):
            return Response(
                {"detail": "op_type_id is inactive."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for ph in practice_hours:
            for dh in doctor_hours:
                ...
                window_start_dt = timezone.make_aware(
                    datetime.combine(start_date, window_start_t), tz
                )
