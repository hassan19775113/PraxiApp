...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ops = list(
            Operation.objects.using("default").select_related("op_room", "primary_surgeon").all()
        )
        ...
        self.stdout.write(
            f"| Operationen        |{len(ops_today):^11}|{len(ops_week):^11}|{len(ops_month):^11}|{len(ops):^11}|"
        )
        self.stdout.write(
            "+" + "-" * 20 + "+" + "-" * 11 + "+" + "-" * 11 + "+" + "-" * 11 + "+" + "-" * 11 + "+"
        )
        ...
        self.stdout.write(
            f'| Durchschnitt            |{avg_dur:>10.1f} min| {"#" * int(avg_dur / max_bar * 20):<25}|'
        )
        self.stdout.write(
            f'| Median                  |{median_dur:>10.1f} min| {"#" * int(median_dur / max_bar * 20):<25}|'
        )
        self.stdout.write(
            f'| Minimum                 |{min_dur:>10.1f} min| {"#" * int(min_dur / max_bar * 20):<25}|'
        )
        ...
        room_utils = [
            (
                r.name,
                room_minutes.get(r.name, 0),
                room_minutes.get(r.name, 0) / total_capacity * 100,
            )
            for r in rooms
        ]
        ...
        if room_utils:
            ...
            self.stdout.write(
                f'| Durchschnitt       |          |{avg_util:>8.1f}% | {"~" * int(avg_util / max(max_util, 1) * 20):<24}|'
            )
        self.stdout.write("+" + "-" * 20 + "+" + "-" * 10 + "+" + "-" * 10 + "+" + "-" * 25 + "+")
        self.stdout.write("")
        ...
        for h, c in sorted_hours:
            ...
            self.stdout.write(
                f'|  {h:02d}:00   |{c:^10}| {"#" * int(c / max_h * 35):<36}{peak:>8}|'
            )
        self.stdout.write("+" + "-" * 10 + "+" + "-" * 10 + "+" + "-" * 45 + "+")
        self.stdout.write("")
        ...
        for d, c in sorted_days:
            ...
            self.stdout.write(
                f'|    {day_names[d]:<5} |{c:^10}| {"#" * int(c / max_d * 35):<36}{peak:>8}|'
            )
        self.stdout.write("+" + "-" * 10 + "+" + "-" * 10 + "+" + "-" * 45 + "+")
        self.stdout.write("")
        ...
        for s, c, a in surgeon_stats[:6]:
            ...
            self.stdout.write(
                f'| {s:<16} |{c:^8}|{a:>8.1f} min| {"#" * int(c / max_s * 20):<19}{peak:>7}|'
            )
        self.stdout.write("+" + "-" * 18 + "+" + "-" * 8 + "+" + "-" * 12 + "+" + "-" * 27 + "+")
        self.stdout.write("")
        ...
        for r, c, a in room_stats:
            ...
            self.stdout.write(
                f'| {r:<16} |{c:^8}|{a:>8.1f} min| {"#" * int(c / max_r * 20):<19}{peak:>7}|'
            )
        self.stdout.write("+" + "-" * 18 + "+" + "-" * 8 + "+" + "-" * 12 + "+" + "-" * 27 + "+")
        self.stdout.write("")
        ...
        if sorted_hours:
            self.stdout.write(
                f"  * Peak-Stunde:          {sorted_hours[0][0]:02d}:00 ({sorted_hours[0][1]} OPs)"
            )
        if sorted_days:
            self.stdout.write(
                f"  * Peak-Tag:             {day_names[sorted_days[0][0]]} ({sorted_days[0][1]} OPs)"
            )
        if surgeon_stats:
            self.stdout.write(
                f"  * Top-Chirurg:          {surgeon_stats[0][0]} ({surgeon_stats[0][1]} OPs)"
            )
        self.stdout.write("=" * 70)
