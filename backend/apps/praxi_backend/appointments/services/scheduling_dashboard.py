...
class DashboardContext:
    ...
    def load_data(self):
        ...
        self.doctors = list(
            User.objects.using("default").filter(role__name="doctor").select_related("role")
        )

        self.rooms = list(Resource.objects.using("default").filter(type="room", active=True))
        ...
        self.absences = list(
            ...
            .filter(start_date__lte=self.target_date, end_date__gte=self.target_date)
            .select_related("doctor")
        )

        self.breaks = list(
            DoctorBreak.objects.using("default")
            .filter(date=self.target_date)
            .select_related("doctor")
        )
    ...
    def _create_rooms(self):
        ...
        for i in range(3):
            room, _ = Resource.objects.using("default").get_or_create(
                name=f"OP-Saal {base}_{i+1}", defaults={"type": "room", "active": True}
            )
            self.rooms.append(room)

    def _create_types(self):
        self.apt_type, _ = AppointmentType.objects.using("default").get_or_create(
            name=f"Untersuchung_{self.seed}", defaults={"duration_minutes": 30, "color": "#3B82F6"}
        )
    ...
    def _calculate_stats(self):
        ...
        if self.appointments:
            durations = [
                (a.end_time - a.start_time).total_seconds() / 60 for a in self.appointments
            ]
        ...
        for doc in self.doctors:
            ...
            doc_ops = [
                ...
                if o.primary_surgeon_id == doc.id or getattr(o, "assistant_id", None) == doc.id
            ]
        ...
        for absence in self.absences:
            for apt in self.appointments:
                ...
                if (
                    apt.doctor_id == absence.doctor_id
                    and absence.start_date <= apt_date <= absence.end_date
                ):
...
def generate_daily_overview(ctx: DashboardContext) -> str:
    ...
    lines.append("├─────────────────────────────────────────────────────────────┤")
    lines.append(
        f"│  Termine: {ctx.stats.total_appointments:<5}  │  Operationen: {ctx.stats.total_operations:<5}            │"
    )
    lines.append("├─────────────────────────────────────────────────────────────┤")
    ...
    total_conflicts = (
        ctx.stats.doctor_conflicts
        + ctx.stats.room_conflicts
        + ctx.stats.working_hours_violations
        + ctx.stats.absence_conflicts
    )
...
def generate_weekly_overview(ctx: DashboardContext) -> str:
    ...
    lines.append("┌─────────────────────────────────────────────────────────────┐")
    lines.append(
        f"│  WOCHENÜBERSICHT: {ctx.week_start.strftime('%d.%m.')} - {ctx.week_end.strftime('%d.%m.%Y'):<27}│"
    )
    lines.append("├───────┬────────┬─────┬──────────────────────────────────────┤")
    ...
    for i in range(7):
        ...
        lines.append(
            f"│{is_target}{weekdays[i]:<4} │{day_apts:^8}│{day_ops:^5}│ {severity} {heatmap}                  │"
        )
...
def generate_resource_summary(ctx: DashboardContext) -> str:
    ...
    avg_doc_util = 0
    if ctx.doctor_stats:
        avg_doc_util = sum(ds.utilization_pct for ds in ctx.doctor_stats.values()) / len(
            ctx.doctor_stats
        )
    ...
    if ctx.doctor_stats:
        ...
        lines.append(
            f"│    Höchste:  {max_doc.doctor_name[:15]:<15} ({max_doc.utilization_pct:5.1f}%)               │"
        )
        lines.append(
            f"│    Niedrigste: {min_doc.doctor_name[:15]:<15} ({min_doc.utilization_pct:5.1f}%)             │"
        )
    ...
    avg_room_util = 0
    if ctx.room_stats:
        avg_room_util = sum(rs.utilization_pct for rs in ctx.room_stats.values()) / len(
            ctx.room_stats
        )
    ...
    if ctx.room_stats:
        ...
        lines.append(
            f"│    Höchste: {max_room.room_name[:16]:<16} ({max_room.utilization_pct:5.1f}%)              │"
        )
...
def generate_kpis(ctx: DashboardContext) -> str:
    ...
    lines.append(
        f"│ Ø Terminlänge            │ {ctx.stats.avg_appointment_duration:>6.1f} min              │"
    )
    lines.append(
        f"│ Ø OP-Dauer               │ {ctx.stats.avg_operation_duration:>6.1f} min              │"
    )
