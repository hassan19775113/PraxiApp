...
from praxi_backend.appointments.exceptions import DoctorAbsentError, WorkingHoursViolation
...
def detect_doctor_conflict(ctx: ReportContext) -> ConflictDetail:
    ...
    return ConflictDetail(
        ...
        affected_objects=[
            ...
            {
                "type": "NewAppointment",
                "time": f"{start2.strftime('%H:%M')}-{end2.strftime('%H:%M')}",
            },
        ],
        time_window={
            "date": str(monday),
...
def generate_summary(conflicts: list[ConflictDetail]) -> ConflictSummary:
    ...
    if summary.by_category.get("doctor_absent", 0) > 0:
        summary.recommendations.append("Integrieren Sie Abwesenheitskalender in die Terminplanung")
...
def format_text_report(report: ConflictReport) -> str:
    ...
    for conf in report.all_conflicts:
        prio_icon = (
            "🔴"
            if conf.priority == ConflictPriority.HIGH
            else "🟡" if conf.priority == ConflictPriority.MEDIUM else "🟢"
        )
