...
class AppointmentValidatorsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.resource = Resource.objects.using("default").create(
            name="Room 1", type="room", active=True
        )
        self.inactive_resource = Resource.objects.using("default").create(
            name="Room X", type="room", active=False
        )
    ...
    def test_validate_within_working_hours_ok_and_unavailable(self):
        ...
        validate_within_working_hours_or_unavailable(
            doctor=self.doctor, start_time=start_ok, end_time=end_ok
        )
        ...
        with self.assertRaises(serializers.ValidationError) as ctx:
            validate_within_working_hours_or_unavailable(
                doctor=self.doctor, start_time=start_bad, end_time=end_bad
            )
