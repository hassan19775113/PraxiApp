...
class AppointmentServicesTest(TestCase):
    ...
    def test_apply_overlap_date_filters_day(self):
        ...
        start_out = timezone.make_aware(
            datetime.combine(day + timezone.timedelta(days=1), time(10, 0)), tz
        )
        end_out = timezone.make_aware(
            datetime.combine(day + timezone.timedelta(days=1), time(10, 30)), tz
        )
        Appointment.objects.using("default").create(
            patient_id=2,
            type=self.type,
        ...
        filtered = apply_overlap_date_filters(
            qs, date_str=str(day), start_date_str=None, end_date_str=None
        )
    ...
    def test_apply_overlap_date_filters_range(self):
        ...
        start_out = timezone.make_aware(
            datetime.combine(end_day + timezone.timedelta(days=1), time(10, 0)), tz
        )
        end_out = timezone.make_aware(
            datetime.combine(end_day + timezone.timedelta(days=1), time(10, 30)), tz
        )
        Appointment.objects.using("default").create(
            patient_id=4,
            type=self.type,
