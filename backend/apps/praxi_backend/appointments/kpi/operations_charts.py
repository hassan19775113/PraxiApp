"""Charts for the operations dashboard.

Moved 1:1 from `praxi_backend.dashboard.operations_charts` in Phase 2F.
...
def get_utilization_gauge_chart(utilization: float) -> dict[str, Any]:
	"""Gauge-/Doughnut-Chart der Gesamtauslastung."""
...
def get_resource_utilization_chart(days: int = 30) -> dict[str, Any]:
	"""Horizontales Balkendiagramm für Ressourcenauslastung."""
	from praxi_backend.appointments.kpi.operations_kpis import calculate_resource_utilization, get_date_range as kpi_date_range

	_, _, start_dt, end_dt = kpi_date_range(days)
	resources = calculate_resource_utilization(start_dt, end_dt, days)

	all_resources = resources["resources"][:10]
...
def get_room_heatmap_chart(days: int = 30) -> dict[str, Any]:
	"""Heatmap-Daten für Raumauslastung nach Wochentag und Stunde."""
	start_dt, end_dt = get_date_range(days)
	...
	for room in rooms[:5]:
		room_data = {"room_id": room.id, "room_name": room.name, "room_color": room.color, "data": []}

		appointments = (
			Appointment.objects.using("default")
			.filter(start_time__gte=start_dt, start_time__lte=end_dt, resources=room)
			.annotate(weekday=ExtractWeekDay("start_time"), hour=ExtractHour("start_time"))
			.values("weekday", "hour")
			.annotate(count=Count("id"))
		)
		...
		operations = (
			Operation.objects.using("default")
			.filter(start_time__gte=start_dt, start_time__lte=end_dt, op_room=room)
			.annotate(weekday=ExtractWeekDay("start_time"), hour=ExtractHour("start_time"))
			.values("weekday", "hour")
			.annotate(count=Count("id"))
		)
...
def get_status_distribution_chart(days: int = 30) -> dict[str, Any]:
	...
	return {
		"labels": labels,
		"datasets": [
			{"data": data, "backgroundColor": colors, "borderWidth": 2, "borderColor": "#FFFFFF"}
		],
	}


def get_daily_trend_chart(days: int = 30) -> dict[str, Any]:
	"""Liniendiagramm für täglichen Trend."""
	start_dt, end_dt = get_date_range(days)
...
def get_documentation_chart(documentation_data: dict[str, Any]) -> dict[str, Any]:
	"""Chart für Dokumentationsstatus."""
	return {
...
def get_services_table(services_data: list[dict[str, Any]]) -> dict[str, Any]:
	"""Tabellendaten für Leistungsübersicht."""
	return {
...
def get_all_operations_charts(days: int = 30, kpis: dict[str, Any] | None = None) -> dict[str, Any]:
	...
	if kpis:
		if "utilization" in kpis:
			charts["utilization_gauge"] = get_utilization_gauge_chart(kpis["utilization"].get("utilization", 0))
