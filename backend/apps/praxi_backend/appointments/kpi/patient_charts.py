...
def get_key_labs_chart(patient_id: int) -> dict[str, Any]:
    ...
    for lab_key, color in zip(key_labs, colors):
        ...
        datasets.append(
            {"label": name, "data": values, "borderColor": color, "fill": False, "tension": 0.3}
        )
        ...
        timeline.append(
            {
                ...
                "severity_color": {"high": "#DC3545", "medium": "#FFC107", "low": "#28A745"}.get(
                    d.get("severity", "low")
                ),
            }
        )
...
def get_appointment_history_chart(patient_id: int, months: int = 12) -> dict[str, Any]:
    ...
    appointments = (
        ...
        .filter(patient_id=patient_id, start_time__gte=start_date)
        .order_by("start_time")
    )
...
def get_appointment_timeline(patient_id: int, limit: int = 20) -> list[dict]:
    ...
    past = (
        Appointment.objects.using("default")
        .filter(patient_id=patient_id, start_time__lt=now)
        .order_by("-start_time")[: limit // 2]
    )

    future = (
        Appointment.objects.using("default")
        .filter(patient_id=patient_id, start_time__gte=now)
        .order_by("start_time")[: limit // 2]
    )
