"""Charts for the doctors dashboard.

Moved 1:1 from `praxi_backend.dashboard.doctor_charts` in Phase 2F.
...
from praxi_backend.appointments.kpi.doctor_kpis import (
...
def get_appointment_volume_chart(days: int = 30) -> dict[str, Any]:
	...
	return {
		"labels": labels,
		"datasets": [
			{"label": "Abgeschlossen", "data": completed_data, "backgroundColor": "#28A745"},
			{"label": "Geplant", "data": scheduled_data, "backgroundColor": "#007BFF"},
			{"label": "Storniert", "data": cancelled_data, "backgroundColor": "#DC3545"},
		],
...
def get_no_show_comparison_chart(days: int = 30) -> dict[str, Any]:
	...
	return {
		"labels": labels,
		"datasets": [
			{"label": "Erschienen (%)", "data": show_data, "backgroundColor": "#28A745"},
			{"label": "No-Show (%)", "data": no_show_data, "backgroundColor": "#DC3545"},
		],
	}
...
def get_weekly_volume_chart(doctor_id: int | None = None, weeks: int = 12) -> dict[str, Any]:
	...
	base_query = Appointment.objects.using("default").filter(start_time__gte=start_dt, start_time__lte=end_dt)

	if doctor_id:
		base_query = base_query.filter(doctor_id=doctor_id)

	weekly_data = (
		base_query.annotate(week=TruncWeek("start_time"))
		.values("week")
...
def get_documentation_comparison_chart(days: int = 30) -> dict[str, Any]:
	...
	return {
		...
		"datasets": [
			{"label": "Abgeschlossen", "data": completed_data, "backgroundColor": "#28A745"},
			{"label": "Offen", "data": pending_data, "backgroundColor": "#FFC107"},
			{"label": "Überfällig", "data": overdue_data, "backgroundColor": "#DC3545"},
		],
	}
...
def get_new_patient_comparison_chart(days: int = 30) -> dict[str, Any]:
	...
	from praxi_backend.appointments.kpi.doctor_kpis import calculate_new_patient_rate
	...
	return {
		"labels": labels,
		"datasets": [
			{"label": "Neupatienten", "data": new_data, "backgroundColor": "#17A2B8"},
			{"label": "Bestandspatienten", "data": returning_data, "backgroundColor": "#6C757D"},
		],
	}


def get_services_table_data(doctor_id: int):
	"""Generiert Tabellendaten für Leistungsübersicht."""
	from praxi_backend.appointments.kpi.doctor_kpis import _generate_demo_services
	...
	return {"services": services, "total_count": total_count, "total_points": total_points}
