...
def _generate_demo_labs(patient_id: int) -> list[dict]:
	...
	labs = []
	lab_dates = [today - timedelta(days=7), today - timedelta(days=30), today - timedelta(days=90), today - timedelta(days=180)]
...
def calculate_patient_status(patient_id: int) -> PatientStatus:
	...
	has_any_recent = Appointment.objects.using("default").filter(patient_id=patient_id, start_time__gte=twelve_months_ago).exists()
	...
	if recent_30_days >= 3:
		return PatientStatus(status="in_treatment", label="In Behandlung", color="#FFC107", icon="🏥")
...
def calculate_appointment_activity(patient_id: int) -> dict[str, Any]:
	...
	appointments = Appointment.objects.using("default").filter(patient_id=patient_id, start_time__gte=year_ago)
...
def calculate_lab_traffic_light(patient_id: int) -> dict[str, Any]:
	...
	if not labs:
		return {"latest_date": None, "values": {}, "summary": {"green": 0, "yellow": 0, "red": 0}, "history": []}
	...
	return {"latest_date": latest["date"], "values": evaluated_values, "summary": summary, "history": labs}
...
def get_patient_profile(patient_id: int) -> dict[str, Any] | None:
	...
	if patient is None:
		...
		first_names = ["Alex", "Sam", "Taylor", "Jordan", "Robin", "Casey", "Jamie", "Morgan", "Chris", "Kai"]
		last_names = [
			"Müller",
			"Schmidt",
			"Schneider",
...
def get_patient_overview_stats() -> dict[str, Any]:
	...
	today_start = timezone.make_aware(datetime.combine(now.date(), datetime.min.time()), timezone.get_current_timezone())
	today_end = today_start + timedelta(days=1)

	today_patients = (
		Appointment.objects.using("default")
		.filter(start_time__gte=today_start, start_time__lt=today_end, status__in=["scheduled", "confirmed"])
		.values_list("patient_id", flat=True)
		.distinct()
		.count()
	)
