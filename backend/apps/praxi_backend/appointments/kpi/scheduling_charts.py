...
Moved 1:1 from `praxi_backend.dashboard.scheduling_charts` in Phase 2F.
...
from praxi_backend.appointments.kpi.scheduling_kpis import (
    calculate_doctor_capacity_utilization,
    calculate_room_capacity_utilization,
    calculate_scheduling_efficiency,
...
def get_utilization_trend_chart(days: int = 30) -> dict[str, Any]:
    ...
    daily_counts = (
        ...
        .filter(start_time__gte=start_date, status__in=["scheduled", "confirmed", "completed"])
...
def get_completion_rate_trend() -> dict[str, Any]:
    ...
    no_show_data = [
        round((w["no_show"] / w["total"]) * 100, 1) if w["total"] > 0 else 0 for w in weeks
    ]
    cancellation_data = [
        round((w["cancelled"] / w["total"]) * 100, 1) if w["total"] > 0 else 0 for w in weeks
    ]
...
def get_doctor_utilization_chart() -> dict[str, Any]:
    ...
    return {
        "labels": labels,
        "datasets": [
            {"label": "Auslastung %", "data": data, "backgroundColor": colors, "borderRadius": 8}
        ],
    }
...
def get_hourly_load_chart() -> dict[str, Any]:
    ...
    return {
        "labels": labels,
        "datasets": [
            {"label": "Termine", "data": data, "backgroundColor": colors, "borderRadius": 4}
        ],
    }
...
def get_weekday_load_chart() -> dict[str, Any]:
    ...
    return {
        "labels": day_names,
        "datasets": [
            {"label": "Termine", "data": data, "backgroundColor": colors, "borderRadius": 8}
        ],
    }
