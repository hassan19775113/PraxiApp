...
class AppointmentCalendarDayView(View):
    ...
    def get(self, request):
        ...
        for appt in rows:
            ...
            doctor_name = appt.doctor.get_full_name() or getattr(
                appt.doctor, "username", str(appt.doctor_id)
            )
            type_name = appt.type.name if appt.type else "Termin"

            accent = None
            try:
                accent = getattr(appt.type, "color", None) or getattr(
                    appt.doctor, "calendar_color", None
                )
            except Exception:
                accent = None
        ...
        context = {
            ...
            "selected_doctor_name": (
                (_doctor_display_name(selected_doctor)) if selected_doctor is not None else ""
            ),
...
class AppointmentCalendarWeekView(View):
    ...
    def get(self, request):
        ...
        context = {
            ...
            "selected_doctor_id": selected_doctor_id,
            "selected_doctor_name": (
                _doctor_display_name(selected_doctor) if selected_doctor else ""
            ),
            "doctors": _unique_doctor_items(doctors_qs=doctors_qs, limit=60),
            "start_min": start_min,
            "end_min": end_min,
...
class AppointmentCalendarMonthView(View):
    ...
    def get(self, request):
        ...
        for appt in rows:
            ...
            type_name = appt.type.name if appt.type else "Termin"
            accent = (
                getattr(appt.type, "color", None)
                or getattr(appt.doctor, "calendar_color", None)
                or "#0078D4"
            )
            evt = {
                "id": appt.id,
