...
# Repo root: this script lives in infrastructure/scripts/
REPO_ROOT = Path(__file__).resolve().parents[2]

# Django code lives in REPO_ROOT/backend after reorg
...
def main():
    ...
    print("=" * 60)
    print("  PraxiApp Deployment Check")
    ...
    if static_root and Path(static_root).exists():
        results.append(("Static Files", "OK", "STATIC_ROOT exists"))
    elif static_root:
    ...
    required_packages = ["corsheaders", "rest_framework", "rest_framework_simplejwt"]
    missing = [pkg for pkg in required_packages if find_spec(pkg) is None]
    if missing:
        results.append(("Dependencies", "FAIL", f"Missing: {missing}"))
    else:
