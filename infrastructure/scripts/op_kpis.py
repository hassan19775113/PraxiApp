...
from praxi_backend.appointments.models import Operation, Resource  # noqa: E402
...
if durations:
    ...
    n = len(sorted_dur)
    median_dur = (
        sorted_dur[n // 2] if n % 2 == 1 else (sorted_dur[n // 2 - 1] + sorted_dur[n // 2]) / 2
    )
...
room_utils = [
    (r.name, room_minutes.get(r.name, 0), room_minutes.get(r.name, 0) / total_capacity * 100)
    for r in rooms
]
...
for s, ol in surgeon_ops.items():
    count = len(ol)
    durs = [
        (o.end_time - o.start_time).total_seconds() / 60 for o in ol if o.end_time and o.start_time
    ]
