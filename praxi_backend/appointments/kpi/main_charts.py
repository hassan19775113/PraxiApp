"""Main dashboard chart payloads.

Moved 1:1 from `praxi_backend.dashboard.charts` in Phase 2F.
...
def get_operations_per_doctor() -> dict[str, Any]:
	...
	ops_by_surgeon = (
		...
		.values(
			...
			"primary_surgeon__calendar_color",
		)
		.annotate(count=Count("id"))
		.order_by("-count")
	)
...
def get_room_utilization_chart() -> dict[str, Any]:
	...
	from praxi_backend.appointments.kpi.main_kpis import calculate_room_utilization
...
def get_hourly_heatmap() -> dict[str, Any]:
	...
	for day_idx, day_name in enumerate(day_names):
		for hour in range(24):
			...
			if count > 0:
				datasets.append({"x": hour, "y": day_idx, "v": count})
...
def get_all_charts() -> dict[str, Any]:
	...
	logger.debug('kpi.main.get_all_charts start')
	with timed_block('kpi.main.get_all_charts', log=logger, level='debug'):
		result = {
	...
	logger.debug('kpi.main.get_all_charts end')
	return result
