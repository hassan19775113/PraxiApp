...
from django.db.models.functions import ExtractHour, ExtractWeekDay, TruncDate, TruncWeek  # noqa: F401
...
def calculate_available_slots(start_date: date, end_date: date) -> dict[str, Any]:
	...
	practice_hours = list(
		PracticeHours.objects.using("default").filter(active=True).values("weekday", "start_time", "end_time")
	)
...
def calculate_no_show_rate() -> dict[str, Any]:
	...
	past_appointments = Appointment.objects.using("default").filter(end_time__lt=now, start_time__gte=last_30_start)

	total = past_appointments.count()

	# No-Show = Termine die nicht completed und nicht cancelled sind
	no_shows = past_appointments.filter(status__in=["scheduled", "confirmed"]).count()
	...
	prev_past = Appointment.objects.using("default").filter(end_time__lt=prev_30_end, start_time__gte=prev_30_start)
...
def calculate_completion_rate() -> dict[str, Any]:
	...
	appointments = Appointment.objects.using("default").filter(start_time__gte=last_30_start, start_time__lte=last_30_end)
...
def calculate_cancellation_rate() -> dict[str, Any]:
	...
	cancelled = Appointment.objects.using("default").filter(created_at__gte=last_30_start, status="cancelled").count()
...
def calculate_rebooking_rate() -> dict[str, Any]:
	...
	return {"rebooking_rate": rebooking_rate, "rebooked_count": rebooked, "total_appointments": total}
...
def calculate_new_patient_conversion() -> dict[str, Any]:
	...
	patient_first_appointments = (
		Appointment.objects.using("default").values("patient_id").annotate(first_appt=Min("start_time")).order_by("patient_id")
	)
...
def calculate_doctor_capacity_utilization() -> list[dict[str, Any]]:
	...
	doctors = User.objects.using("default").filter(is_active=True, role__name="doctor").order_by("last_name", "first_name")
...
def get_scheduling_trends() -> dict[str, Any]:
	...
	for i in range(12):
		...
		appointments = Appointment.objects.using("default").filter(start_time__gte=week_start, start_time__lt=week_end)
		total = appointments.count()
		completed = appointments.filter(status="completed").count()
		cancelled = appointments.filter(status="cancelled").count()

		no_show = appointments.filter(status__in=["scheduled", "confirmed"], end_time__lt=now).count()
