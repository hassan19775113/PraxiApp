"""Main dashboard KPI calculations.

Moved 1:1 from `praxi_backend.dashboard.kpis` in Phase 2F.
...
from praxi_backend.appointments.models import Appointment, DoctorHours, Operation, PracticeHours, Resource
...
def get_user_kpis() -> dict[str, Any]:
	...
	total_doctors = User.objects.using("default").filter(is_active=True, role__name="doctor").count()
	total_assistants = User.objects.using("default").filter(is_active=True, role__name="assistant").count()
	total_admins = User.objects.using("default").filter(is_active=True, role__name="admin").count()
...
def get_patient_count() -> int:
	...
	try:
		appt_count = int(
			Appointment.objects.using("default").values("patient_id").distinct().count()
		)
	except Exception:
		appt_count = 0
...
def get_appointment_kpis() -> dict[str, Any]:
	...
	appointments_week = (
		Appointment.objects.using("default")
		.filter(start_time__gte=week_start, start_time__lte=week_end)
		.count()
	)

	appointments_month = (
		Appointment.objects.using("default")
		.filter(start_time__gte=month_start, start_time__lte=month_end)
		.count()
	)
	...
	_ = (
		Appointment.objects.using("default")
		.filter(start_time__gte=last_30_start)
		.annotate(duration_mins=(models.F("end_time") - models.F("start_time")))
	...
	if appointments.exists():
		total_minutes = sum((a.end_time - a.start_time).total_seconds() / 60 for a in appointments)
	...
	appointments_prev_week = (
		Appointment.objects.using("default")
		.filter(start_time__gte=prev_week_start, start_time__lte=prev_week_end)
		.count()
	)
...
def calculate_room_utilization() -> list[dict[str, Any]]:
	...
	for room in rooms:
		...
		ar_rows = (
			...
			.filter(resource=room, appointment__start_time__gte=week_start, appointment__start_time__lte=week_end)
...
def get_peak_hours() -> dict[str, Any]:
	...
	return {"hourly": hourly_dict, "peak_hour": peak_hour, "peak_count": peak_count}
...
def get_peak_days() -> dict[str, Any]:
	...
	return {"daily": daily_dict, "peak_day": peak_day, "peak_count": peak_count}


def get_all_kpis() -> dict[str, Any]:
	"""Sammelt alle KPIs für das Dashboard."""
	logger.debug('kpi.main.get_all_kpis start')
	with timed_block('kpi.main.get_all_kpis', log=logger, level='debug'):
		result = {
			"users": get_user_kpis(),
	...
	logger.debug('kpi.main.get_all_kpis end')
	return result
