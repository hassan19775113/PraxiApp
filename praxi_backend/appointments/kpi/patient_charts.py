...
Moved 1:1 from `praxi_backend.dashboard.patient_charts` in Phase 2F.
...
from praxi_backend.appointments.kpi.patient_kpis import (
...
def get_spo2_chart(patient_id: int) -> dict[str, Any]:
	...
	return {
		...
		"reference_lines": [{"label": "Normal (min)", "value": 95, "color": "#FFC107"}],
	}
...
def get_all_vitals_chart(patient_id: int) -> dict[str, Any]:
	...
	def normalize(values: list, min_val: float, max_val: float) -> list:
		return [round((v - min_val) / (max_val - min_val) * 100, 1) for v in values]
...
def get_key_labs_chart(patient_id: int) -> dict[str, Any]:
	...
	for lab_key, color in zip(key_labs, colors):
		...
		datasets.append({"label": name, "data": values, "borderColor": color, "fill": False, "tension": 0.3})
...
def get_lab_status_summary(patient_id: int) -> dict[str, Any]:
	...
	return {"date": latest["date"], "values": values, "summary": summary}
...
def get_diagnoses_by_severity_chart(patient_id: int) -> dict[str, Any]:
	...
	return {
		...
		"datasets": [
			{
				"label": "Diagnosen",
				"data": [len(by_severity["high"]), len(by_severity["medium"]), len(by_severity["low"])],
		...
		timeline.append(
			{
				...
				"severity_color": {"high": "#DC3545", "medium": "#FFC107", "low": "#28A745"}.get(d.get("severity", "low")),
			}
		)
...
def get_appointment_history_chart(patient_id: int, months: int = 12) -> dict[str, Any]:
	...
	appointments = Appointment.objects.using("default").filter(patient_id=patient_id, start_time__gte=start_date).order_by("start_time")
	...
	return {
		"labels": labels,
		"datasets": [
			{"label": "Wahrgenommen", "data": [monthly_data[m]["completed"] for m in labels], "backgroundColor": "#28A745"},
			{"label": "Storniert", "data": [monthly_data[m]["cancelled"] for m in labels], "backgroundColor": "#6C757D"},
			{"label": "No-Show", "data": [monthly_data[m]["no_show"] for m in labels], "backgroundColor": "#DC3545"},
		],
	}
...
def get_appointment_timeline(patient_id: int, limit: int = 20) -> list[dict]:
	...
	past = (
		Appointment.objects.using("default").filter(patient_id=patient_id, start_time__lt=now).order_by("-start_time")[: limit // 2]
	)

	future = (
		Appointment.objects.using("default").filter(patient_id=patient_id, start_time__gte=now).order_by("start_time")[: limit // 2]
	)
...
def get_medication_compliance_chart(patient_id: int) -> dict[str, Any]:
	...
	return {
		"labels": labels,
		"datasets": [{"label": "Compliance (%)", "data": compliance_values, "backgroundColor": colors, "borderWidth": 0}],
	}
...
def get_all_patient_charts(patient_id: int) -> dict[str, Any]:
	...
	return {
		...
		"diagnoses": {"by_severity": get_diagnoses_by_severity_chart(patient_id), "timeline": get_diagnoses_timeline(patient_id)},
		"appointments": {"history": get_appointment_history_chart(patient_id), "timeline": get_appointment_timeline(patient_id)},
		"medications": {"compliance": get_medication_compliance_chart(patient_id), "timeline": get_medication_timeline(patient_id)},
	}
