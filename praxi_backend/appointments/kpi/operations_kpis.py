"""KPIs for the operations dashboard.

Moved 1:1 from `praxi_backend.dashboard.operations_kpis` in Phase 2F.
...
def _generate_demo_patient_flow_times(appointment_count: int, seed: int = 42) -> dict[str, Any]:
...
def _generate_demo_documentation(appointment_count: int, seed: int = 42) -> dict[str, Any]:
	...
	return {
		...
		"avg_completion_time": round(random.uniform(2, 24), 1),
	}
...
def calculate_overall_utilization(start_dt: datetime, end_dt: datetime, days: int) -> dict[str, Any]:
	...
	for appt in appointments:
		booked_minutes += (appt.end_time - appt.start_time).total_seconds() / 60
	...
	for op in operations:
		booked_minutes += (op.end_time - op.start_time).total_seconds() / 60
...
def calculate_patient_throughput(start_dt: datetime, end_dt: datetime, days: int) -> dict[str, Any]:
	"""Berechnet den Patientendurchsatz."""
	completed_appointments = (
		Appointment.objects.using("default")
		.filter(start_time__gte=start_dt, start_time__lte=end_dt, status="completed")
		.count()
	)

	completed_operations = (
		Operation.objects.using("default").filter(start_time__gte=start_dt, start_time__lte=end_dt, status="done").count()
	)
...
def calculate_no_show_rate(start_dt: datetime, end_dt: datetime) -> dict[str, Any]:
	...
	scheduled_past = (
		Appointment.objects.using("default")
		.filter(start_time__gte=start_dt, start_time__lte=past_end, status__in=["scheduled", "confirmed"])
		.count()
	)

	completed = (
		Appointment.objects.using("default")
		.filter(start_time__gte=start_dt, start_time__lte=past_end, status="completed")
		.count()
	)

	total_past = scheduled_past + completed
	no_shows = scheduled_past
...
def calculate_cancellation_rate(start_dt: datetime, end_dt: datetime) -> dict[str, Any]:
	"""Berechnet die Stornoquote."""
	total = Appointment.objects.using("default").filter(start_time__gte=start_dt, start_time__lte=end_dt).count()

	cancelled = (
		Appointment.objects.using("default")
		.filter(start_time__gte=start_dt, start_time__lte=end_dt, status="cancelled")
		.count()
	)
	...
	return {"cancellation_rate": cancelled_rate, "cancelled_count": cancelled, "total": total}


def calculate_resource_utilization(start_dt: datetime, end_dt: datetime, days: int) -> dict[str, Any]:
	...
	for resource in resources:
		...
		if resource.type == "room":
			...
			for ar in appt_resources:
				booked_minutes += (ar.appointment.end_time - ar.appointment.start_time).total_seconds() / 60
			...
			for od in op_devices:
				booked_minutes += (od.operation.end_time - od.operation.start_time).total_seconds() / 60
...
def calculate_bottleneck_index(resource_utilization: dict[str, Any], no_show: dict[str, Any], cancellation: dict[str, Any]) -> dict[str, Any]:
	"""Berechnet den Engpass-Index."""
	high_util_resources = [r for r in resource_utilization["resources"] if r["utilization"] >= 80]
	...
	bottleneck_index = round(resource_score * 0.5 + no_show_score * 0.3 + cancel_score * 0.2, 1)
...
def calculate_hourly_distribution(start_dt: datetime, end_dt: datetime) -> dict[str, Any]:
	...
	hourly = {h: {"hour": h, "appointments": 0, "operations": 0, "total": 0} for h in range(6, 22)}
...
def calculate_status_distribution(start_dt: datetime, end_dt: datetime) -> dict[str, Any]:
...
def calculate_patient_flow_status(start_dt: datetime, end_dt: datetime) -> dict[str, Any]:
	...
	flow_status = (
		...
		.filter(status_changed_at__gte=today_start)
		.values("status")
		.annotate(count=Count("id"))
	)
...
def get_all_operations_kpis(days: int = 30) -> dict[str, Any]:
	...
	logger.debug('kpi.operations.get_all_operations_kpis start (days=%s)', days)
	with timed_block('kpi.operations.get_all_operations_kpis', log=logger, level='debug'):
	...
	logger.debug('kpi.operations.get_all_operations_kpis end (days=%s)', days)
	return result
