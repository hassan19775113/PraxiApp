...
def favicon_view(request):
    ...
    minimal_ico = (
        ...
        + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2  # Row 5 (cross horizontal)
        + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2  # Row 6
        + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2  # Row 7
        + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2  # Row 8
        + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2  # Row 9
        + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2  # Row 10
        + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2  # Row 11
        + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xFF\xFF\xFF\xFF' * 2 + b'\xE2\x4A\x90\xFF' * 4 + b'\xFF\xFF\xFF\xFF' * 2  # Row 12
...
urlpatterns = [
    ...
    path("praxi_backend/dashboard/", include("praxi_backend.dashboard.urls")),  # Dashboard (MUSS VOR praxi_backend/ stehen!)
    path(
        "praxi_backend/core/user/",
        RedirectView.as_view(url="/praxi_backend/Dashboardadministration/core/user/", permanent=False),
    ),
    path(
        "praxi_backend/appointments/operation/",
        RedirectView.as_view(url="/praxi_backend/Dashboardadministration/appointments/operation/", permanent=False),
    ),
    path("praxi_backend/Dashboardadministration/", praxi_admin_site.urls),  # Custom PraxiApp Admin
    path("praxi_backend/", RedirectView.as_view(url="/praxi_backend/Dashboardadministration/", permanent=False)),
