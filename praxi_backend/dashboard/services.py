...
def build_resources_dashboard_context() -> dict:
	...
	resources = (
		Resource.objects.using("default").filter(active=True).order_by("type", "name")
	)
		...
		payload.append(
			{
				...
				"name": (get_doctor_profile(d).get("name") if d is not None else _doctor_full_name(d)),
...
def build_doctors_dashboard_context(
	*,
	...
	base_ops = Operation.objects.using("default").filter(start_time__gte=start_dt, start_time__lte=end_dt)
	...
	context["charts_json"] = json.dumps(
		{
			"utilization_by_doctor": {
				...
				"datasets": [{"label": "Auslastung (%)", "data": [row["utilization"] for row in doctors_overview]}],
			}
		}
	)
	context["title"] = "Ärzte-Dashboard"
...
def build_doctors_compare_context(*, doctor1_id: int | None, doctor2_id: int | None, days: int) -> dict:
	...
	context["doctor_list"] = [
		{"id": d.id, "name": get_doctor_profile(d).get("name")}
...
def build_patients_overview_context() -> dict:
		...
		for patient in patients:
			try:
				...
				try:
					...
					status_dict = {"label": status.label, "color": status.color, "icon": status.icon}
				except Exception:
					status_dict = {"label": "Unbekannt", "color": "#7A8A99", "icon": "○"}

				try:
					risk = calculate_patient_risk_score(patient.id)
					risk_dict = {"score": risk["score"], "level": risk["level"], "color": risk["level_color"]}
				except Exception:
	...
	if not patient_list:
		try:
			patient_ids = (
				Appointment.objects.using("default").values_list("patient_id", flat=True).distinct()[:100]
			)
					...
					patient_list.append(
						{
							...
							"status": {"label": status.label, "color": status.color, "icon": status.icon},
							"risk": {"score": risk["score"], "level": risk["level"], "color": risk["level_color"]},
						}
					)
				except Exception:
					continue
...
def build_patients_nav_payload(*, limit: int = 50) -> list[dict]:
	...
	try:
		...
		payload = [{"patient_id": p.id, "display_name": f"{p.last_name}, {p.first_name}"} for p in patients]
		if payload:
			return payload
	except Exception:
		pass
...
def search_patients_payload(*, query: str, limit: int = 10) -> dict:
	...
	try:
		...
		results = [
			{"id": p.id, "name": f"{p.last_name}, {p.first_name}", "birth_date": p.birth_date.isoformat()}
			for p in patients
		]
...
def create_patient_note(*, patient_id: int, author_name: str, author_role: str, content: str) -> None:
	if not content:
		return
