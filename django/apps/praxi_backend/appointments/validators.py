...
def validate_positive_int(
    value, *, field_name: str, message_required: str, message_invalid: str, message_positive: str
) -> int:
    if value is None:
...
def validate_doctor_self_only(*, request_user: User | None, doctor: User | None) -> None:
    ...
    if user_role == "doctor" and getattr(doctor, "id", None) != getattr(request_user, "id", None):
        raise serializers.ValidationError(
            {"doctor": "Ärzte dürfen nur eigene Termine anlegen/ändern."}
        )
...
def dedupe_int_list(raw_values: list, *, field_name: str) -> list[int]:
    ...
    for v in raw_values or []:
        try:
            ...
            raise serializers.ValidationError(
                {field_name: f"{field_name} muss eine Liste von Ganzzahlen sein."}
            )
        if iv not in seen:
            seen.add(iv)
            unique.append(iv)
...
def resolve_active_resources(resource_ids: list[int]) -> list[Resource]:
    if not resource_ids:
        return []
    resources = list(
        Resource.objects.using("default").filter(id__in=resource_ids, active=True).order_by("id")
    )
    found_ids = {r.id for r in resources}
    missing = [rid for rid in resource_ids if rid not in found_ids]
    if missing:
        raise serializers.ValidationError(
            {"resource_ids": "resource_ids enthält unbekannte oder inaktive Ressource(n)."}
        )
    return resources
...
def resolve_active_devices(device_ids: list[int]) -> list[Resource]:
    ...
    if missing:
        raise serializers.ValidationError(
            {"op_device_ids": "op_device_ids enthält unbekannte/inaktive/nicht-Gerät-Ressource(n)."}
        )
    return devices
...
def doctor_unavailable_payload(
    *, doctor: User, local_start_dt: datetime, local_end_dt: datetime
) -> dict:
...
def raise_doctor_unavailable(*, doctor: User, start_time: datetime, end_time: datetime) -> None:
    ...
    raise serializers.ValidationError(
        doctor_unavailable_payload(
            doctor=doctor, local_start_dt=local_start, local_end_dt=local_end
        )
    )


def validate_within_working_hours_or_unavailable(
    *, doctor: User, start_time: datetime, end_time: datetime
) -> None:
