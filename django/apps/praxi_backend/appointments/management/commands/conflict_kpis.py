...
class Command(BaseCommand):
    ...
    def detect_conflicts(self):
        ...
        appointments = list(
            Appointment.objects.using("default").select_related("doctor", "type").all()
        )
        operations = list(
        ...
        for appt in appointments:
            if appt.start_time:
                ...
                if hour < 7 or hour >= 20:
                    conflicts.append({"type": "edge_cases", "hour": hour, "items": [appt]})
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"| Konflikte / 100 Termine      |{conf_per_100_appts:>10.1f}  | {rating(conf_per_100_appts):<21} |"
        )
        self.stdout.write(
            f"| Konflikte / 100 OPs          |{conf_per_100_ops:>10.1f}  | {rating(conf_per_100_ops):<21} |"
        )
        self.stdout.write(
            f"| Konflikte / 100 Gesamt       |{conf_per_100_total:>10.1f}  | {rating(conf_per_100_total):<21} |"
        )
        self.stdout.write("+" + "-" * 30 + "+" + "-" * 12 + "+" + "-" * 23 + "+")
        self.stdout.write("")
        ...
        doctors = {
            u.id: u.username for u in User.objects.using("default").filter(role__name="doctor")
        }
        ...
        self.stdout.write(
            "| Konflikt-Typ              | Anzahl |   %    | Visualisierung         |"
        )
        self.stdout.write("+" + "-" * 25 + "+" + "-" * 8 + "+" + "-" * 8 + "+" + "-" * 24 + "+")
        ...
        self.stdout.write("+" + "-" * 68 + "+")
        self.stdout.write(
            "  Legende: [ ]=0  [.]=wenig  [o]=mittel  [O]=hoch  [#]=sehr hoch  [@]=max"
        )
        self.stdout.write("")
        ...
        if type_counts:
            ...
            self.stdout.write(
                f"  * Haeufigster Typ:      {type_labels.get(top_type[0], top_type[0])} ({top_type[1]})"
            )
