...
def calculate_overall_utilization(
    start_dt: datetime, end_dt: datetime, days: int
) -> dict[str, Any]:
...
def calculate_patient_throughput(start_dt: datetime, end_dt: datetime, days: int) -> dict[str, Any]:
    ...
    completed_operations = (
        ...
        .filter(start_time__gte=start_dt, start_time__lte=end_dt, status="done")
        .count()
    )
    ...
    for resource in resources:
        ...
        if resource.type == "room":
            ...
            for ar in appt_resources:
                booked_minutes += (
                    ar.appointment.end_time - ar.appointment.start_time
                ).total_seconds() / 60
            ...
            for od in op_devices:
                booked_minutes += (
                    od.operation.end_time - od.operation.start_time
                ).total_seconds() / 60
...
def calculate_bottleneck_index(
    resource_utilization: dict[str, Any], no_show: dict[str, Any], cancellation: dict[str, Any]
) -> dict[str, Any]:
