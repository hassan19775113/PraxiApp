...
from praxi_backend.appointments.models import Appointment, DoctorAbsence, Operation, Resource
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        operations = list(
            Operation.objects.using("default").select_related("primary_surgeon", "op_room").all()
        )
        ...
        for name, appts, ops, mins, util in doc_utils:
            ...
            self.stdout.write(
                f"| {name:<14} |{appts:^8}|{ops:^8}|{util:>8.1f}% | {bar:<15}{status:>6}|"
            )
        ...
        if doc_utils:
            ...
            self.stdout.write(
                f'| Durchschnitt   |        |        |{avg_util:>8.1f}% | {"~" * int(avg_util / max(max_util, 1) * 15):<21}|'
            )
        ...
        for name, ops, mins, util in room_utils:
            ...
            self.stdout.write(
                f"| {name:<16} |{ops:^10}|{mins:>8.0f}  |{util:>8.1f}% | {bar:<11}{status:>5}|"
            )
        ...
        if room_utils:
            ...
            self.stdout.write(
                f'| Durchschnitt     |          |          |{avg_room_util:>8.1f}% | {"~" * int(avg_room_util / max(max_room_util, 1) * 12):<16}|'
            )
        ...
        for name, appts, ops, mins, util in doc_utils:
            ...
            self.stdout.write(
                f"| {name:<14} |{mins:>8.0f} min|{avail:>8} min| [{bar_used}{bar_free}] {usage_pct:>5.1f}%|"
            )
        ...
        for doc in doctors:
            ...
            self.stdout.write(
                f"| {doc.username:<14} |{days:^12}|{quote:>10.1f}% | [{bar}{bar_ok}]{status:>6}|"
            )
        ...
        self.stdout.write(
            f"| Termin-Slots (gesamt)        |{total_available_slots:>13}  |                    |"
        )
        self.stdout.write(
            f"| OP-Slots (gesamt)            |{total_op_slots:>13}  |                    |"
        )
        self.stdout.write(
            f"| Gesamt verfuegbar            |{total_slots:>13}  |                    |"
        )
        self.stdout.write("+" + "-" * 30 + "+" + "-" * 15 + "+" + "-" * 20 + "+")
        self.stdout.write(
            f"| Termine gebucht              |{total_appts:>13}  |                    |"
        )
        self.stdout.write(
            f"| OPs gebucht                  |{total_ops:>13}  |                    |"
        )
        self.stdout.write(
            f"| Gesamt belegt                |{total_booked:>13}  |                    |"
        )
        self.stdout.write("+" + "-" * 30 + "+" + "-" * 15 + "+" + "-" * 20 + "+")
        ...
        self.stdout.write(
            f"| Belegungsrate                |{booking_rate:>12.1f}%  | [{bar_booked}{bar_free}] |"
        )
        self.stdout.write(
            f"| Freie Slots                  |{free_slots:>13}  |                    |"
        )
        self.stdout.write("+" + "-" * 30 + "+" + "-" * 15 + "+" + "-" * 20 + "+")
        self.stdout.write("")
